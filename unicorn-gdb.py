...
def hookCode(mu, address, size, user_data):
    ...
    if currentLR != lastLR:
        print("LR changed, probably function called: %08x (thumb=%d)" % (
        address, int(mu.query(UC_QUERY_MODE) == UC_MODE_THUMB)))
    ...
    if address == ADDRESS + 0x664 or address == 0x10664:
        ...
        print("%08x calling __copy(dst=%x, src=%x, len=%x, a4=%x)" % (
        address, srcAddr, dstAddr, srcLen, mu.reg_read(UC_ARM_REG_R3)))
 ...
 #       mu.emu_stop()
    ...
    if address == 0x1068C:
        value = codecs.decode("%08x" % (mu.reg_read(UC_ARM_REG_R1) - mu.reg_read(UC_ARM_REG_R2) - 1), "hex")[::-1]
...
mu.mem_map(0x1C000000,
           0x1000000)  # On this CPU the 0x1C00A000 - 0x1C00AFFF range appears to be mapped to various ports and test pads all around the PCB.
...
mu.mem_write(0x1C00A000 + 0xA0, b"\x1E")  # fix for 0x906 - 0x90C
mu.mem_write(0x1C00A000 + 0x62C, b"\xff")  # fix for 0x1e6 - 0x1ec
# mu.mem_write(0x1C00A000+0xC58, b"\xff") # fix for 0x1e6 - 0x1ec

# __copy needs to have a4 = 1 to activate block-copy (and not the weird memory[0x...] shit)
mu.mem_write(0x7fffffb8, b"\x01\x00\x00\x00")  # stack #0x48+var_48 = 1
# but a4 will be overwritten by sub_81E unless these values are set
# mu.mem_write(0x4005B60, "\x01\x00\x00\x00")
mu.mem_write(0x4005B60, b"\x3A\x7C\x37\x54")

mu.mem_write(0x1C004A0C, b"\x01\x00\x00\x00")

# mu.mem_write(0x4005B64, "\x00\xF6\x05\x00")
mu.mem_write(0x4005B64, b"\x00\xF6\xEF\xFF")

mu.mem_write(0x1C002E14, b"\xff\xff\xff\xff")
...
hook_mem_fetch = mu.hook_add(UC_HOOK_MEM_FETCH_UNMAPPED | UC_HOOK_MEM_FETCH_PROT | UC_HOOK_MEM_FETCH_INVALID,
                             hookInvalidFetchMemory, None)
